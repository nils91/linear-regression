...
public class DrawPane extends JPanel {
	...
	private void drawCoordinateSystem(Graphics g) {
		if(coordinateSystemOriginOnScreen!=null) {
			...
			//scale
			//x
			for(int i=coordinateSystemInternalOrigin.x+xScale;i<coordinateSystemInternalPosSize.width;i+=xScale) {
				g.drawLine(mapXAxis(i), coordinateSystemOriginOnScreen.y-2, mapXAxis(i), coordinateSystemOriginOnScreen.y+2);
			}
			//y
			for(int i=coordinateSystemInternalOrigin.y+yScale;i<coordinateSystemInternalPosSize.height;i+=yScale) {
				g.drawLine(coordinateSystemOriginOnScreen.x-2,mapYAxis(i), coordinateSystemOriginOnScreen.x+2, mapYAxis(i));
			}
			//points
...
//line
g.drawLine(mapXAxis(0),mapYAxis((int) alrf.getY(0)) , mapXAxis(coordinateSystemInternalPosSize.width), mapYAxis((int) alrf.getY(coordinateSystemInternalPosSize.width)));
		}
	...
	private int mapXAxis(int v) {
		return coordinateSystemOriginOnScreen.x+(int) map(v, coordinateSystemInternalPosSize.width, getSize().width-coordinateSystemOriginOnScreen.x);
	}
	private int mapYAxis(int v) {
		return coordinateSystemOriginOnScreen.y-(int) map(v, coordinateSystemInternalPosSize.height, coordinateSystemOriginOnScreen.y);
	}
